  ...
  if (User::exists_by_id($system_core, $user_id)) {
    ...
    if (isset($user_password) && isset($user_password_repeat)) {
      if (!empty($user_password) || !empty($user_password_repeat)) {
        if ($user_password == $user_password_repeat) {
          ...
          $handler_message = (!isset($handler_message)) ? 'Данные пользователя не были сохранены, поскольку пароли не совпадают.' : $handler_message;
    ...
    if (isset($user_email)) {
      if (filter_var($user_email, FILTER_VALIDATE_EMAIL)) {
        ...
        $handler_message = (!isset($handler_message)) ? 'Данные пользователя не были сохранены, поскольку e-mail имеет неверный формат.' : $handler_message;
        $handler_status_code = (!isset($handler_status_code)) ? 0 : $handler_status_code;
      }
    }
    ...
    if ($user_is_updated) {
      $handler_message = (!isset($handler_message)) ? 'Данные пользователя успешно сохранены.' : $handler_message;
      $handler_status_code = (!isset($handler_status_code)) ? 1 : $handler_status_code;
    } else {
      $handler_message = (!isset($handler_message)) ? 'Данные пользователя не были сохранены, поскольку произошел неизвестный сбой.' : $handler_message;
      $handler_status_code = (!isset($handler_status_code)) ? 0 : $handler_status_code;
    }
  } else {
