      ...
      if (strlen($comment_content) >= 16) {
        if (Entry::exists_by_id($system_core, $comment_entry_id)) {
          ...
          if (!is_null($comment)) {
            ...
            $handler_message = (!isset($handler_message)) ? sprintf('API ERROR: %s', $system_core->locale->get_single_value_by_key('API_PUT_DATA_SUCCESS')) : $handler_message;
